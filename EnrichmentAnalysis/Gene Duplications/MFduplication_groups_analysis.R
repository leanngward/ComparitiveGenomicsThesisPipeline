...
## USE TOPGO FEATURES ####
  ...
  #Group 1 - testing for ontology MF
  GOgroup_1_MF <- new("topGOdata", ontology = 'MF', allGenes = cands1,
                       annot = annFUN.gene2GO, gene2GO = geneID2GO)


  #Group 2 - testing for ontology MF
  GOgroup_2_MF <- new("topGOdata", ontology = 'MF', allGenes = cands2,
                      annot = annFUN.gene2GO, gene2GO = geneID2GO)


  #Group 3 - testing for ontology MF
  GOgroup_3_MF <- new("topGOdata", ontology = 'MF', allGenes = cands3,
                      annot = annFUN.gene2GO, gene2GO = geneID2GO)


  #Group 4 - testing for ontology MF, MF
  GOgroup_4_MF <- new("topGOdata", ontology = 'MF', allGenes = cands4,
                     annot = annFUN.gene2GO, gene2GO = geneID2GO)


  #Group 5 - testing for ontology MF
  GOgroup_5_MF <- new("topGOdata", ontology = 'MF', allGenes = cands5,
                     annot = annFUN.gene2GO, gene2GO = geneID2GO) 
  
  #Group 6 - testing for ontology MF
  GOgroup_6_MF <- new("topGOdata", ontology = 'MF', allGenes = cands6,
                      annot = annFUN.gene2GO, gene2GO = geneID2GO)  
...
#'weight01' is topgo's default algorithm
  ...
  f_res_1_MF = runTest(GOgroup_1_MF, algorithm='weight01', statistic='fisher')
  f_res_2_MF = runTest(GOgroup_2_MF, algorithm='weight01', statistic='fisher')
  f_res_3_MF = runTest(GOgroup_3_MF, algorithm='weight01', statistic='fisher')
  f_res_4_MF = runTest(GOgroup_4_MF, algorithm='weight01', statistic='fisher')
  f_res_5_MF = runTest(GOgroup_5_MF, algorithm='weight01', statistic='fisher')
  f_res_6_MF = runTest(GOgroup_6_MF, algorithm='weight01', statistic='fisher')
  
  
  #Define terms that are available for analysis using usedGO() so that we can access the length of our relative result list

  allGO_1_MF = usedGO(GOgroup_1_MF)
  allGO_2_MF = usedGO(GOgroup_2_MF)
  allGO_3_MF = usedGO(GOgroup_3_MF)
  allGO_4_MF = usedGO(GOgroup_4_MF)
  allGO_5_MF = usedGO(GOgroup_5_MF)
  allGO_6_MF = usedGO(GOgroup_6_MF)
...
#This is the data we need to calculate upregulation and downregulation of our significant terms.
  ...
  ###### GENE ONTOLOGY TERMS ########
  res_1_MF = GenTable(GOgroup_1_MF, f = f_res_1_MF, orderBy='f',topNodes=length(allGO_1_MF))
  res_2_MF = GenTable(GOgroup_2_MF, f = f_res_2_MF, orderBy='f',topNodes=length(allGO_2_MF))
  res_3_MF = GenTable(GOgroup_3_MF, f = f_res_3_MF, orderBy='f',topNodes=length(allGO_3_MF))
  res_4_MF = GenTable(GOgroup_4_MF, f = f_res_4_MF, orderBy='f',topNodes=length(allGO_4_MF))
  res_5_MF = GenTable(GOgroup_5_MF, f = f_res_5_MF, orderBy='f',topNodes=length(allGO_5_MF))
  res_6_MF = GenTable(GOgroup_6_MF, f = f_res_6_MF, orderBy='f',topNodes=length(allGO_6_MF))
...
#Start with the significant results from res_ objects calculated above.
  ...
  sig_1_MF <- res_1_MF[res_1_MF$f < .05, ]
  sig_2_MF <- res_2_MF[res_2_MF$f < .05, ]
  sig_3_MF <- res_3_MF[res_3_MF$f < .05, ]
  sig_4_MF <- res_4_MF[res_4_MF$f < .05, ]
  sig_5_MF <- res_5_MF[res_5_MF$f < .05, ]
  sig_6_MF <- res_6_MF[res_6_MF$f < .05, ]

  
  sig_1_MF$FC <- ifelse(sig_1_MF$Expected > sig_1_MF$Significant, -1 * (sig_1_MF$Expected / sig_1_MF$Significant) ,sig_1_MF$Significant / sig_1_MF$Expected)

  sig_2_MF$FC <- ifelse(sig_2_MF$Expected > sig_2_MF$Significant, -1 * (sig_2_MF$Expected / sig_2_MF$Significant) ,sig_2_MF$Significant / sig_2_MF$Expected)     

  sig_3_MF$FC <- ifelse(sig_3_MF$Expected > sig_3_MF$Significant, -1 * (sig_3_MF$Expected / sig_3_MF$Significant) ,sig_3_MF$Significant / sig_3_MF$Expected)

  sig_4_MF$FC <- ifelse(sig_4_MF$Expected > sig_4_MF$Significant, -1 * (sig_4_MF$Expected / sig_4_MF$Significant) ,sig_4_MF$Significant / sig_4_MF$Expected)

  sig_5_MF$FC <- ifelse(sig_5_MF$Expected > sig_5_MF$Significant, -1 * (sig_5_MF$Expected / sig_5_MF$Significant) ,sig_5_MF$Significant / sig_5_MF$Expected)
  
  sig_6_MF$FC <- ifelse(sig_6_MF$Expected > sig_6_MF$Significant, -1 * (sig_6_MF$Expected / sig_6_MF$Significant) ,sig_6_MF$Significant / sig_6_MF$Expected)

  #Label each set's group name 
  sig_1_MF$group <- group1
  sig_2_MF$group <- group2
  sig_3_MF$group <- group3
  sig_4_MF$group <- group4
  sig_5_MF$group <- group5
  sig_6_MF$group <- group6

  #Combine groups you want to compare for the visualization
  mydf <- rbind(sig_1_MF, sig_2_MF, sig_3_MF,  sig_4_MF, sig_5_MF, sig_6_MF)   
...
#Create a gene universe with all GOterms
 ...
 #term2go should be a data.frame with first column of term ID and second column of corresponding mapped gene
  ...
  formula_res_clus <- clusterProfiler::compareCluster(Entrez ~ othergroup+group, data=mydf_clus, 
                                                    fun= "enricher", TERM2GENE = term2go, TERM2NAME = NA, pAdjustMethod = "BH",
                                                    pvalueCutoff = 0.05, 
                                                    universe = NA,
                                                    minGSSize = 1,
                                                    maxGSSize = 1000,
