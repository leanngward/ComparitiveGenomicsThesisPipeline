...
##### 6. Updates to parallel runs

	1. Create your control files using one of the previous scripts. (.sh files not necessary for this)
	2. Run: python3 v2_make_list_parallel.py [input directory] [string flag] [output file]
		This script creates a list separated by new-line characters with the control files in the output file.
	3. Run: python3 v2_submit_jobs_from_list_parallel.py [input list] [output file]
		Creates a .sh files that runs GNU parallel. GNU parallel calls yn00/codeml/hyphy
		with the with control files in the provided list. As of 8/15/2022, this particular
		version runs yn00 with that format.

		For models that need multiple input files (hyphy, for example). The inputs files
		must be separated by commas on each line in the list.
			file1,file2
			nextfile1,nextfil2
		Use GNU parallel options --col-sep ',' and {1},{2}...
