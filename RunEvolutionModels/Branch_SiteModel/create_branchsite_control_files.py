...
def codeml_branches_runner(codeml_prefix,seq_file,treefile,outfile,gene_name,groupName):
    ...
    control_file_name = codeml_prefix  + gene_name + '_'+grouName+'_branchsite_alt.ctl'
...
for items in dirList:
	...
	if(os.path.isdir(geneDirName)):
		...
		if mnmFlag == False:
			...
			altoutFileName = dirName+geneName+"/"+geneName+"_"+groupName+"_alt_branchsite.out"
		else:
			seqFileName = dirName+geneName+"/edited_sites_nuc_"+geneName+"_aligned.phy"
			altoutFileName = dirName+geneName+"/"+geneName+"_"+gropuName+"_alt_mnm_branchsite.out"
