---
title: "enrichment_analysis_omegas_method2"
author: "LeAnn Ward"
date: "`r Sys.Date()`"
...
## Create appropriate variables and run topGO ####
  ...
  #Custom annotations - for each gene identifier are listed GO terms to which this gene is specifically annotated
    ...
    #Get a geneuniverse where TRUE = foreground > background and FALSE = background > foreground
      geneuniverse <- as.numeric(subset$Alt.Omega.1 > subset$Alt.Omega.0.Background)
      names(geneuniverse) <- subset$Group
      
      length(which(geneuniverse == T))
      length(which(geneuniverse == F))
